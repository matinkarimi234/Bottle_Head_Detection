...
class UsbCameraSettings:
    ...
    # Usually MJPG gives better FPS for USB cameras.
    # Try "MJPG" first. If it fails, try "YUYV".
    fourcc: str = "MJPG"

    disable_auto_exposure: bool = True
    disable_auto_white_balance: bool = True

    # USB camera exposure values are camera-dependent.
    # Some cameras use negative values, some use positive values.
    # Use v4l2-ctl --list-ctrls to check your camera range.
    ...
    # White balance temperature, for example 4000, 4500, 5000.
...
class UsbCameraSource:
    """
    USB camera source using OpenCV VideoCapture.

    Output:
        frame_bgr: OpenCV BGR image, shape H x W x 3

    Usage:
        settings = UsbCameraSettings()
        cam = UsbCameraSource(settings)
        cam.open()
        ok, frame = cam.read()
        cam.close()
    """
    ...
    def _apply_opencv_settings(self) -> None:
        ...
        # OpenCV auto exposure is inconsistent between cameras.
        # On many Linux UVC cameras:
        #   1 = manual mode
        #   3 = auto mode
        ...
        # OpenCV white balance control is also inconsistent.
    ...
    def _apply_v4l2_controls_before_open(self) -> None:
        """
        More reliable than OpenCV for USB camera controls.

        Different cameras expose different controls. This function tries common
        UVC control names. If a control is unsupported, it simply prints a warning
        and continues.
        """
        ...
        if self.settings.disable_auto_exposure:
            # Common UVC values:
            # exposure_auto=1 means manual mode
            # exposure_auto=3 means aperture priority / auto mode
    ...
    def print_current_settings(self) -> None:
        ...
        print(f"  AE disabled : {self.settings.disable_auto_exposure}")
        print(f"  AWB disabled: {self.settings.disable_auto_white_balance}")
