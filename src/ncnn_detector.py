...
class NcnnYoloSettings:
    ...
    target_class_names: Optional[Tuple[str, ...]] = None
...
class NcnnYoloDetector:
    ...
    def load(self) -> None:
        ...
        if ret_param != 0 or ret_bin != 0:
            self.net = None
            raise RuntimeError(f"Failed to load NCNN model. param={ret_param}, bin={ret_bin}")
    ...
    def unload(self) -> None:
        if self.net is not None:
            print("Unloading NCNN model...")
        self.net = None
        gc.collect()

    def detect(self, frame_bgr: np.ndarray) -> List[Detection]:
        if self.net is None:
            raise RuntimeError("NCNN model is not loaded. Call load() first.")
        ...
        letterboxed, scale, pad_x, pad_y = self._letterbox(frame_bgr, self.settings.img_size)
        ...
        with self.net.create_extractor() as ex:
            ...
            if ret != 0:
                raise RuntimeError(f"NCNN extract failed for output '{self.output_name}', ret={ret}")
    ...
    def _resolve_input_output_names(self, param_path: Path) -> None:
        ...
        try:
            ...
            if self.input_name is None and input_names:
                self.input_name = input_names[0]
            if self.output_name is None and output_names:
    ...
    @staticmethod
    def _letterbox(image_bgr: np.ndarray, target_size: int, color=(114, 114, 114)):
    ...
    @staticmethod
    def _make_ncnn_input(image_bgr_square: np.ndarray):
    ...
    def _decode_yolo_output(
        ...
        if self.settings.target_class_ids is not None:
            target_ids = set(int(x) for x in self.settings.target_class_ids)
            keep = keep & np.array([int(c) in target_ids for c in class_ids], dtype=bool)

        if self.settings.target_class_names is not None:
            target_names = set(str(x).lower() for x in self.settings.target_class_names)
            name_keep = []
            for c in class_ids:
                c_int = int(c)
                name = self.class_names[c_int] if 0 <= c_int < len(self.class_names) else str(c_int)
                name_keep.append(str(name).lower() in target_names)
            keep = keep & np.array(name_keep, dtype=bool)
